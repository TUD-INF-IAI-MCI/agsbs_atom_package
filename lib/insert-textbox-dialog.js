...
export default class InsertTextboxDialog extends DialogView {
    ...
    textFrameCheckbox.addEventListener('change', function() {
      ...
      if(textBoxCheckbox.checked){
        textBoxCheckbox.checked = !textBoxCheckbox.checked;
				...
				textboxContent.disabled = !textboxContent.disabled;
    ...
    allColorsLanguage.forEach(function(color){
      ...
      switch(color.trim()){
        case "colorRed":
          value = language.colorRed;
          break;
        case "colorBlack":
          value = language.colorBlack;
          break;
        case "colorGreen":
          value = language.colorGreen
          break;
        case "colorYellow":
          value = language.colorYellow;
          break;
        case "colorOrange":
          value = language.colorOrange;
          break;
        default:
          break;
      }
    ...
    const selectTextboxColor = viewManager.addDropDownMenu(insertTextboxForm, 'textbox_category', language.color, colors);
    ...
    const textboxContent = viewManager.addTextarea(insertTextboxForm, 'insert_textbox_content', language.insertTextboxContent);
		...
		textboxContent.addEventListener('change', function(event){
			main.changeSubmitEnabled();
		});
		textboxContent.disabled = true;
    ...
    insertTextboxForm.appendChild(insertTextboxSubmit);
		...
		insertTextboxForm.addEventListener('submit', function(event){
			...
			if(textBoxCheckbox.checked){
				divClass = "textBox";
				comment = language.textBox;
			}else{
				divClass = "textFrame";
				comment = language.textFrame;
			}
			let color = selectTextboxColor.value;
			editor.insertAuthorAnnotation(`${language.begin} ${comment} ${color}`);
			editor.insertDiv(divClass, selectTextboxColor.value);
			editor.insertAuthorAnnotation(`${language.end} ${comment} ${color}`);
		}
