...
export default class InsertTextboxDialog extends DialogView {
	...
	changeSubmitEnabled(){
		textIsOptional = atom.config.get('agsbs-atom-package.optionalTextboxContent')
		if(!this.textFrameCheckbox.checked && !this.textBoxCheckbox.checked &&
			 !this.annotationCheckbox.checked || !textIsOptional
			 && this.textboxContent.value.length < 5)
		{
			this.viewManager.disableButton(this.insertTextboxSubmit);
		}else if(textIsOptional || this.textboxContent.value.length >= 5){
