    ...
    abortButton.addEventListener('click', function(event) {
      ...
      activePanels.forEach(function (currentValue, index, originalArray){
        if(currentValue.visible){
          ...
          console.log("JUST FOR TESTIN REMOVE NUMBERS");
          numbers = '[{"2": "|| - Slide - 97 -"},{ "18": "|| - Slide 98 -"}]';
          editor.correctWrongPageNumbers(numbers);
          console.log("show list with wrong number");
        }
      });
    });
      ...
      activePanels.forEach(function (currentValue, index, originalArray){
        if(currentValue.visible){
          ...
          promise.then(function(result){
            let agsbs = atom.packages.getLoadedPackage('agsbs-atom-package').mainModule;
            let editorFunctions = agsbs.editorFunctions;
            editor.correctWrongPageNumbers(result.out);
          });
        }
      });
    });
