...
import InsertTextboxDialog from './insert-textbox-dialog';
import SaveDialog from './save-dialog';
import WarningPagenumDiaglog from './warning_pagenum_dialog';
...
export default {
	...
	config: {
		...
		gitLocalPath:{
				type: 'string',
				default: iPath.join(atom.config.getUserConfigPath().split(".atom")[0], "Documents","agsbs_git")
		},
		useAutoCorrectionOfPageNumbers:{
			type: 'boolean',
			default: true,
		},
		...
		optionalTextboxContent:{
			title: 'Optional Textbox content',
			description: 'If this is selected, the title and text of a text box is not ' +
										'required and empty containers are inserted.',
			type: 'boolean',
			default: false
		}
	},
	...
	renderGitCloneDialog(){
		this.gitCloneDialog = atom.workspace.addModalPanel({
			item: new GitCloneDialog().getElement(),
			visible: false
		});
		return this.gitCloneDialog;
	},
	...
	renderInsertTextboxDialog(){
		var insertTextboxDialogInstance = new InsertTextboxDialog();
		this.insertTextboxDialog = atom.workspace.addModalPanel({
			item: insertTextboxDialogInstance.getElement(),
			visible: false
		});
		return {panel: this.insertTextboxDialog, instance: insertTextboxDialogInstance};
	},
	...
	renderWarningPagenumberingDialog(){
		var warningPagenumDiaglogInstance = new WarningPagenumDiaglog();
		warningPagenumDiaglogInstance.id = "warningPagenumDiaglog";
		this.warningPagenumDiaglogInstance = atom.workspace.addModalPanel({
			item: warningPagenumDiaglogInstance.getElement(),
			visible: false
		});
		return {panel: this.warningPagenumDiaglogInstance, instance: warningPagenumDiaglogInstance};
	},
	...
	toggle() {
		...
		promise.then(function(result){
			...
			console.log(language.wrongMatucVersion +"\nInstalled Version is \t\t" + json.result.version.toString() +"\nRequired Version is \t\t" + atom.config.get('agsbs-atom-package.supportedMatucVersion'))
