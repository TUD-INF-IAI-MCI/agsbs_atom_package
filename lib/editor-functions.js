...
//markdown editor functions that are used in the footer panel
 export default class EditorFunctions {
	...
	isLabelUsed(label) {
		var editor = atom.workspace.getActivePaneItem();
		for (let r = 0; r < editor.getLineCount(); r++) {
			editor.setCursorBufferPosition({row: r, column: 0});
			editor.selectToEndOfLine();
			let textInLine = editor.getSelectedText();
			console.log(textInLine);
			if (textInLine.includes(label)) {
				console.log('YEEAAAH');
			}
		}
	}
	...
	insertFootnote(label, text) {
		...
		var selectedText = editor.getSelectedText();
		var selectedRange = editor.getSelectedBufferRange();
		var fullText = console.log(editor);
	}
