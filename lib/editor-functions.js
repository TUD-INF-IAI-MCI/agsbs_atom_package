...
//markdown editor functions
  ...
  */
  createGridTable(table, columns, rows, tableHead){
    var editorTable = "";
    return "Grid tables will be support soon";
  }
      ...
      for(i=0; i < rows; i++){
        for(j=0; j< columns;j++){
          if(j==0){
            ...
            editorTable += this.rightAlignmentOfContent(table[i][j], columnWidths[j],
                                                        ColumnWidthOffSet,
                                                        TableColumnSpaces);
          }
        }
  ...
  */
  ...
  /**
  * Calculates the column width depending on the each cell of the cell
  * @param {array} table
