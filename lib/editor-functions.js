 ...
 export default class EditorFunctions {
	...
	blockquote() {
		...
		if (selectedText == '') {
			...
			alert(language.blockquoteError);
			return;
		} else {
	...
	addHorizontalRule() {
		...
		if (selectedText == '') {
			...
			alert(language.addHorizontalRuleError);
		}
	}
	...
	newPage() {
		...
		if (selectedRange.start.row == 0) {
			...
			for (let i = 0; i <= selectedRange.start.row; i++) {
				...
				if (editor.getTextInBufferRange(bufferRange) == '|| -') {
					pattern = '|| - page ' + occurances + ' -';
					editor.setTextInBufferRange(bufferRange, pattern);
					let patternLength = 13;
					if (occurances > 999) {
						patternLength = 16;
					} else if (occurances > 99) {
						patternLength = 15;
					} else if (occurances > 9) {
						patternLength = 14;
					}
					editor.setCursorBufferPosition({row: bufferRange.end.row, column: patternLength});
