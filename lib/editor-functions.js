...
//markdown editor functions
 export default class EditorFunctions {
  ...
  /**
  * number is an array [�, (538, "|| - Slide 98 -"), �]
  **/
  ...
  */
  newPageByMatuc(){
    ...
    editor.save();
    editorFunctions.checkPageNumbers();
    ...
    promise.then(function(result) {
      ...
      var json = JSON.parse(result['out']);
        ...
        let pageNumber  = JSON.parse(result['out']).result.pagenumber;
				editor.insertNewlineAbove();
        editor.setCursorScreenPosition([cursorPosition.row-1,0]);
        editor.selectToEndOfLine();
  ...
  checkPageNumbers(){
    ...
    if (atom.config.get('agsbs-atom-package.useAutoCorrectionOfPageNumbers')){
      ...
      promise.then(function(result){
        const agsbs = atom.packages.getLoadedPackage('agsbs-atom-package').mainModule;
        let editorFunctions = agsbs.editorFunctions;
        editorFunctions.correctWrongPageNumbers(result.out);
      });
    }
  }
...
insertDivText(className="", text="", color=""){
  const agsbs = atom.packages.getLoadedPackage('agsbs-atom-package').mainModule;
  const helper = agsbs.helper;
  const language = agsbs.language;
  if(!helper.isBufferSelected(true)){
  ...
  editor.setTextInBufferRange(range,content);
  this.insertHtmlComment(`${language.addContentHere}`);
