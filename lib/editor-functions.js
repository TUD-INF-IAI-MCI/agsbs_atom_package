 ...
 export default class EditorFunctions {
	...
	createTable(table, tableHead) {
		...
		if (selectedText == '') {
			...
			for (let c = 0; c < columns; c++) {
				let substitude = table[0][c].replace('\n', ' <br> ') + ' ';
				if (substitude.includes('\n- ')) {
					substitude = substitude.slice(0, substitude.indexOf('\n- '));
					substitude += '<ul><li>' + substitude.slice(substitude.indexOf('\n- '), substidude.length);

					substitude += '</ul>';
				}
				editorTable += '| ' + table[0][c].replace('\n', ' <br> ') + ' ';
			}
			editorTable += '|\n';
			if (tableHead) {
				for (let c = 0; c < columns; c++) {
