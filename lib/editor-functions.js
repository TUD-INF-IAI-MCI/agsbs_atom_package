...
//markdown editor functions
 export default class EditorFunctions {
  ...
  /**
  * number is an array [Ö, (538, "|| - Slide 98 -"), Ö]
  **/
  correctWrongPageNumbers(numbers){
    var editor = atom.workspace.getActivePaneItem();
    // number = string, e.g "|| - Slide 98 -"
    // index = line number
    parsedNum = JSON.parse(numbers)
    //number = object
    parsedNum.forEach(function(number){
      let linenum = 0;
      let pagenum = "";
      ...
      if(number.hasOwnProperty(k)){
        pagenum = number[k];
        editor.setCursorScreenPosition([linenum-1,0]);
  ...
  newPageByMatuc(){
    ...
    promise.then(function(result) {
      var json = JSON.parse(result['out']);
        ...
        helper.insertTextforRow(`${pageNumber}`,
                                language.addHorizontalRuleError,
                                language.addHorizontalRuleErrorDetail);
    ...
    if(!helper.isBufferSelected(true)){
      return;
		...
		if (selectedText == '') {
          ...
          var converted = buffer.toString('utf8')
          resolve(papa.parse(converted, {delimiter: atom.config.get('agsbs-atom-package.csvDelimiter')}));
				}).then(function(data){
					let commentRange = {
						start: {
							column: 0,
...
insertDiv(className="", color=""){
  ...
  this.insertHtmlComment(`Anfang ${className}`);
  let content = `\n<div class="${className} ${color}">`; // open div
  editor.setTextInBufferRange(range,content);
  this.insertHtmlComment(`Inhalt hier g√§nzen`);
  this.insertHtmlComment(`Ende ${className}`);
