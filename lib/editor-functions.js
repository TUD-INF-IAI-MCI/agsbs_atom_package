...
//markdown editor functions that are used in the footer panel
 export default class EditorFunctions {
	 ...
	 setBold() {
		 ...
		 if (selectedText.slice(0,2) == '__' && selectedText.slice(length - 2, length) == '__') {
	 ...
	 setItalics() {
		 ...
		 var length = selectedText.length;
		 var range = editor.getSelectedBufferRange();
		 if (selectedText.slice(0,3) == '___' || selectedText.slice(0,2) != '__' && selectedText.slice(0,1) == '_' && selectedText.slice(length - 1, length) == '_') {
	 ...
	 strikethrough() {
		 ...
		 var length = selectedText.length;
		 var range = editor.getSelectedBufferRange();
		 if (selectedText.slice(0,2) == '~~' && selectedText.slice(length - 2, length) == '~~') {
					let unset = selectedText.slice(2,length - 2);
					editor.setTextInBufferRange(range, unset);
				} else {
