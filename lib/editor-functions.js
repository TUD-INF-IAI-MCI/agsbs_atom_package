  ...
  newPageByMatuc(){
    ...
    promise.then(function(result) {
      ...
      var json = JSON.parse(result['out']);
        ...
        pageNumber  = JSON.parse(result['out']).result.pagenumber;
        //cause the cursor is not in a empty column
    ...
    pageNumber = "\n"+pageNumber+"\n"
    editor.setTextInBufferRange(bufferRange, pageNumber);
    //editor.setCursorBufferPosition({row: editor.getSelectedBufferRange().end.row + 1, column: 0});
    editor.getElement().focus();
    editor.save();
    setTimeout("", 1000);
    matuc.fixpnumInPlace();
...
insertAuthorAnnotation(annotation) {
    ...
    if(!helper.isBufferSelected(true)){
      ...
      editor.save();
      let newPos = {row: cursorPosition.row+2,
                    column: cursorPosition.column};
...
insertDivText(className="", title="", textboxContent="", color=""){
  ...
  editor.setTextInBufferRange(range,content);
  let newCursorPosition = {row: editor.getSelectedBufferRange().end.row+1,
