...
export default class Matuc {
	...
	*/
	newPage(){
		...
		if(!helper.isBufferSelected(atom.workspace.getActivePaneItem())){
			...
			console.log("CHANGE matuc_js COMMAND JUST for Dev");
			cmd = `python E:\\Git\\GitHub\\AGSBS-infrastructure\\matuc_js.py addpagenum `
  ...
  /**
	...
	*/
	version(){
		console.log("CHANGE matuc_js COMMAND JUST for Dev");
		cmd = 'python E:\\Git\\GitHub\\AGSBS-infrastructure\\matuc_js.py version';
		return new Promise(function (resolve, reject) {
			iExec(cmd, (error, stdout, stderr) => {
				if (error) {
					return reject(error);
	...
	convertFile() {
		...
		iExec(cmd, (error, stdout, stderr) => {
			if (error) {
				...
				let errorObj = JSON.parse(stdout);
				let message = errorFormatter.formatErrorMessage(errorObj.error.message, false)
				message += "\n\n" + errorFormatter.formatErrorMessage(language.checkFile +" " +path , false);
