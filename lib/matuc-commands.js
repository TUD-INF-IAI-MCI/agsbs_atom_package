  ...
  /**
	...
	*/
	newProject(countOfAppendixChapters, countOfChapters, preface, language, path) {
		...
		var cmd;
		if(process.platform == "win32"){
			cmd = '\"' + atom.config.get('agsbs-atom-package.matucPath') +'\\matuc_js\" ';
		}
	...
	*/
	checkEntireProject() {
 	...
 	// profile is a string blind or visually_impaired
	convertFile(profile) {
		const agsbs = atom.packages.getLoadedPackage('agsbs-atom-package').mainModule;
		const language = agsbs.language;
		var editor = atom.workspace.getActivePaneItem();
		var path = editor.buffer.file.path;
		if(editor.buffer.isModified()) editor.buffer.save();
		var cmd = "";
		...
		iExec(cmd, {env: this.getOsLocale()}, ( error, stdout, stderr) => {
			if (error) {
	...
	convertEntireProject(profile) {
		...
		console.log(cmd);
		iExec(cmd, {env: this.getOsLocale(), cwd: path}, (error, stdout, stderr) => {
			if (error) {
				let fragment = JSON.parse(stdout);
				let message = "";
	...
	getRelativePath(path){
		...
		var dirParent = '';
		if(process.platform == "darwin"){
			dirParent = iPath.dirname(iPath.dirname(path));
			relPath = "." + path.replace(dirParent,"") // format ./bilder/file.jpgs
