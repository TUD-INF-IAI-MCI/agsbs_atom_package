...
export default class InsertraphicDialog extends DialogView {
	...
	addPictureData(){
		...
		var pathPictures = iPath.join(path,'bilder');
		var self = this;
		var itemsInDropDown = this.pictureDrop.options.length;
		console.log(itemsInDropDown);
		fs.exists(pathPictures, function(exists) {
  			if (exists) {
				fs.readdir(pathPictures, function(err, items) {
	    			console.log(items);
					if (itemsInDropDown.length != items.length) {
						for (let i = 0; i < items.length; i++) {
							self.addItemToPictureDropdown(items[i]);
						}
					}
  				});
			}
		});
		//this.addItemToPictureDropdown("test");


	}

	addItemToPictureDropdown(item) {
		this.pictureDrop.options.add(new Option("bild1", item));
	}
}
