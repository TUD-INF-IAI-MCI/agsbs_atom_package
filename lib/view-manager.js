...
export default class ViewManager {
	...
	closeDialog() {
		...
		dialogs.forEach(function(dialog, index) {
			if (dialog.isVisible()) {
				...
				viewManager.toggleDialog(index);
			}
		});
	}

	toggleHtmlPreview(target) {
	...
	addButton(parent, id, icon, clickFunctionCallback, shortcut) {
		...
		button.onclick = function() {
			if (clickFunctionCallback.toString().match(':')) {
				atom.commands.dispatch(button, clickFunctionCallback.toString());
			} else {
