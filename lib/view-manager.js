...
export default class ViewManager {
	...
	*/
	toggleDialog(dialogPanel) {
		...
		let listDialogIds = ["insertLinkDialog","insertTableDialog", "insertGraphicDialog", "insertFootnoteDialog", "insertTextboxDialog"];
	...
	*/
	addTextarea(parent, id, placeholder) {
		...
		textarea.addEventListener('keyup', function(event) {
			...
			// set in styles/*.less
			var setHeight = parseInt(window.getComputedStyle(textarea, null).getPropertyValue('height').replace('px', ''));
			...
			if( setHeight < heightWithLineBreaks){
	...
	* @param {boolean} ownContainer should a own container creates for select
	* @return The number-picker as DOM element.
	*/
	addDropDownMenu (parent, id, label, options, ownContainer = true) {
		//Create container.
		...
		//Add label.
		var DropDownMenuLabel = null;
		if(label){
		DropDownMenuLabel = document.createElement('label');
