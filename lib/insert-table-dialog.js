...
export default class InsertTableDialog extends DialogView {
	...
	constructor(serializedState) {
		...
		function createColumbsAndRows() {
			...
			const setContentWidth = parseInt(window.getComputedStyle(dialogContainer, null).getPropertyValue('min-width').replace('px', '')) - 2*setMargin;
			...
			const rowContainerHeight = parseInt(window.getComputedStyle(tableColumbs, null).getPropertyValue('height').replace('px', '')) + 2*setMargin;
