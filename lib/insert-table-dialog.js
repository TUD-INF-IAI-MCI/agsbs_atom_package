	...
	constructor(serializedState) {
		...
		*/
		function setColumnWidth() {
			const dialogContainerWidth = window.getComputedStyle(this.dialogContainer, null).getPropertyValue('width').replace('px', '');
			//Avoid window overflow.
			if(dialogContainerWidth >= window.innerWidth - 400) {
				this.cellWidth = dialogContainerWidth/this.setColumns - 2*this.setMargin;
			};
