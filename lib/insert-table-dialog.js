...
export default class InsertTableDialog extends DialogView {
	...
	*/
	constructor(serializedState) {
		...
		this.dialogHeadline.innerHTML = language.insertTable;
		...
		tableHeadCheckbox.setAttribute('tabindex', 1);
		tableHeadCheckbox.addEventListener('change', function() {
			if(tableHeadCheckbox.checked) {
				...
				addTableHead();
				//tableRows.dispatchEvent(new Event('input'));
				editColumnsAndRows();
				addTabindex();
			} else {
				removeTableHead();
		...
		// add combobox for selection of table type
		//var tableTypes = ["gridTable", "pipeTable", "simpleTable", "multilineTable"];
		var tableTypes = ["gridTable", "simpleTable", "pipeTable"];
		// this.TableType label
		//const selectTableType = document.createElement('select');
		const selectTableType = viewManager.addDropDownMenu (insertTableForm, 'table_type_select', language.selectTableType, []);
		var tableName = "";
		for(var i = 0; i < tableTypes.length; i++){
			switch(tableTypes[i]){
				case "simpleTable":
					tableName = language.simpleTable;
					break;
				case "pipeTable":
					tableName = language.pipeTable;
					break;
				case "gridTable":
					tableName = language.gridTable;
					break;
				case "multilineTable":
					tableName = language.multilineTable;
					break;
				default:
				 console.log("table type is not supported yet!");
				 break;
			}
			selectTableType.options.add(new Option(tableName, tableTypes[i]));
		}
		selectTableType.parentNode.appendChild(selectTableType);
		selectTableType.parentNode.style.margin = '0 auto';
		selectTableType.parentNode.style.display = 'block';
		selectTableType.parentNode.style.width = '66.6%';
		...
		tableRows.setAttribute('tabindex', 3);
		...
		insertTableSubmit.setAttribute('value',language.insert);
		viewManager.disableButton(insertTableSubmit);
		...
		insertTableForm.addEventListener('submit', function(event) {
			...
			let sel = document.getElementById("table_type_select");
			let type = sel.options[sel.selectedIndex].value;
			editor.createTable(table, type, tableHeadCheckbox.checked);
		...
		*/
		...
		function addTableHead() {
			...
			// 	editColumnsAndRows();
			// };
			const tableHead = document.getElementById('row_0');
			if(tableHead) {
				tableHead.style.fontWeight = 'bold';
				tableHead.style.fontSize = '1.4em';
			};
		...
		function removeTableHead() {
			...
			const tableHead = document.getElementById('row_0');
			if(tableHead) {
				tableHead.style.fontWeight = 'normal';
				tableHead.style.fontSize = '1em';
			};
		};
		...
		*/
		...
		function addTabindex() {
			var table = insertTableForm.getElementsByTagName('textarea');
			for(var i = 0; i < table.length; i++) {
				//+ 4 because there are 3 inputs before the table.
				table[i].setAttribute('tabindex', i + 4);
			};
			insertTableSubmit.setAttribute('tabindex', table.length + 4);
		};
	}
}
