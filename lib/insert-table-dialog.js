...
export default class InsertTableDialog extends DialogView {
	...
	*/
	constructor(serializedState) {
		...
		const tableHeadCheckbox = viewManager.addCheckbox(numberpickerContainer, 'tabel_head_checkbox', language.tableHeadCheckbox);
		...
		function addColumns(row) {
			...
			for(var i = this.currentColumns; i < this.setColumns; i++) {
				...
				if(row.classList[0] == 'head_container') {
					...
					cell = viewManager.addTextInput (undefined, 'field_' + i, language.field + ' ' + rowNumber + '.' + (i + 1));
