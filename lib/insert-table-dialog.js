...
export default class InsertTableDialog extends DialogView {
	...
	constructor(serializedState) {
		...
		function createColumbsAndRows() {
			...
			const currentContentWidth = insertTableDialog.dialogContent.offsetWidth;
			const currentMargin = window.getComputedStyle(insertTableDialog.dialogContent, null).getPropertyValue('margin-left').replace('px', '');
			//Manipulate default style.
			const dialogContainer = insertTableDialog.dialogContent.parentElement;
			//Let the Container grow an shrink with its content.
			dialogContainer.style.display = 'table-cell';
			insertTableDialog.dialogContent.style.display = 'table-cell';
			////Repair lost styles by table-cell.
			dialogContainer.style.paddingLeft = currentMargin + 'px';
			numberpickerContainer.style.width = currentContentWidth;
			...
			var cellWidth;
			if(columbs < 3) {
				cellWidth = currentContentWidth/columbs - 2*currentMargin;
			} else {
				cellWidth = currentContentWidth/3 - 2*currentMargin;
			};
