...
export default class NewProjectDialog extends DialogView {
	...
	constructor(serializedState) {
		...
		const git = new Git('iai8292.inf.tu-dresden.de/srv/git/');
		...
		const newProjectGitUser = viewManager.addTextInput(newProjectForm, 'git', 'user name for git repo (zih login)');
		const newProjectPath = viewManager.addFileInput(newProjectForm, 'directory', 'directory');
		const newProjectPreface = viewManager.addCheckbox(newProjectForm, 'preface', 'preface');
		...
		const newProjectLanguage = viewManager.addSelect(newProjectForm, 'language', 'language', ['en', 'de', 'fr']);
		...
		newProjectForm.onsubmit = function(event) {
			var user, path, chapterCount, appendixChapterCount, preface, language,
				tableOfContents, depthOfTableOfContents, sourceAuthor, title, source;
			event.preventDefault();
			user = newProjectGitUser.value;
