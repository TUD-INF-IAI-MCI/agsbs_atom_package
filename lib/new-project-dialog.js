	...
	constructor(serializedState) {
		...
		useGit.addEventListener('change', function() {
			if(useGit.checked) {
				...
				if(documentPreferencesContainer.style.height != '92px' && newProjectPath.files[0]) {
		...
		newProjectSource.addEventListener('input', function() {
			if ( (newProjectSource.value != '') && (newProjectGitUser.value != '') && (newProjectGitPassword.value != '' ) && newProjectPath.files[0] ) {
				documentPreferencesContainer.style.height = '92px';
			} else {
				if(documentPreferencesContainer.style.height != '0px') {
					documentPreferencesContainer.style.height = '0px';
		...
		newProjectPath.addEventListener('change', function() {
			if( !useGit.checked || ( (newProjectSource.value != '') && (newProjectGitUser.value != '') && (newProjectGitPassword.value != '' ) ) ) {
				documentPreferencesContainer.style.height = '92px';
			};
		});
		...
		const newProjectAppendixChapters = this.viewManager.addNumberPicker(documentPreferencesContainer, 'appendixChapters', language.appendixChapters);
