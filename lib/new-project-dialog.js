...
export default class NewProjectDialog extends DialogView {
	...
	*/
	constructor(serializedState) {
		...
		//matuc init blockquote
		const cloneExistingRepo = this.viewManager.addCheckbox(newProjectForm, 'cloneExistingRepo', language.cloneExistingRepo);
		cloneExistingRepo.addEventListener('change', function() {
			toggleDocumentPreferences();
			toogleTableOfContents();
			toggleDocumentMetadata();
			toogleSubmitButton();
		...
		atom.config.observe('agsbs-atom-package.enableGitAutoCommitAfterSave', function(value) {
			if(value) {
				useGit.removeAttribute('disabled');
				useGit.parentNode.style.display = 'flex';
			} else {
				useGit.setAttribute('disabled', '');
				useGit.parentNode.style.display = 'none';
			};
		});
		...
		newProjectForm.addEventListener('submit', function() {
			if(useGit.checked && cloneExistingRepo.checked && areGitValuesSet()){
				cloneGit();
				atom.project.addPath(newProjectPath.files[0].path);
		...
		function toggleCloneExistingRepo(){
			if(!useGit.checked){
				cloneExistingRepo.checked = false;
			}
		}
		function toggleDocumentPreferences() {
			if ((isPathSet() && !cloneExistingRepo.checked) && (areGitValuesSet() || !useGit.checked)) {
		...
		function toogleSubmitButton() {
			...
			}else if (useGit.checked && cloneExistingRepo.checked && areGitValuesSet())
			{
				viewManager.enableButton(newProjectSubmit);
			}
			else {
		...
		//Function that clone a git repository
		function cloneGit(){
			var source, gitUser;
