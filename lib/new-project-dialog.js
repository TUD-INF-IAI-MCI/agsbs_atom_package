...
export default class NewProjectDialog extends DialogView {
	...
	constructor(serializedState) {
		...
		const newProjectGitPath = this.viewManager.addTextInput(newProjectForm, 'git', this.language.gitPath);
		const newProjectPath = this.viewManager.addFileInput(newProjectForm, 'projectDirectory');
		const newProjectPreface = this.viewManager.addCheckbox(newProjectForm, 'preface', this.language.preface);
		const newProjectChapters = this.viewManager.addNumberPicker(newProjectForm, 'chapters', this.language.chapters);
