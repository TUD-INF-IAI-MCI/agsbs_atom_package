	...
	addTableData(){
		...
		var promiseGetAll = new Promise(function(resolve, reject) {
			fs.exists(pathPicture, function(exists) {
					...
					var formatter = new ErrorMessageFormatter();
					var message = self.language.thereAreNoCsvFileInFolder + " " +
					self.language.addCsvFileToFolder + "\""+pathPicture  +"\"";
					message = formatter.formatErrorMessage(message, false);
					atom.notifications.addError(self.language.ErrorNoCsvFile, {
						detail : message,
						dismissable : true
					});
