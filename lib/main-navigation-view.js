...
export default class MainNavigationView {
	...
	constructor(serializedState) {
		...
		const checkProjectButton = viewManager.addButton(tools, 'check_project', language.checkProject, 'fa-check-square', matuc.checkEntireProject, undefined, 'f4');
		const convertFileButton = viewManager.addButton(tools, 'convert_file', language.convertFile, 'fa-wheelchair-alt', matuc.convertFile);
		const commitChangesButton = viewManager.addButton(tools, 'commit_changes', language.commitChanges, 'fa-git-square', viewManager.toggleDialog, commitChangesDialog);
