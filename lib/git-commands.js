...
export default class Git {
	...
	clone(user, repo, path) {
		iExec(`git clone git+ssh://${user}@iai8292.inf.tu-dresden.de/srv/git/AGSBS_Informatik/${repo} ${path}`, {cwd: path}, (error, stdout, stderr) => {
			if (error) {
	...
	*/
	add(path) {
		iExec('git add *', {cwd: path}, (error, stdout, stderr) => {
  			if (error) {
    			console.error(`exec error: ${error}`);
    			return;
  			}
